...
float_t *read_weights(char *filename)
{
    ...
    //     printf("%04x\n",p_w[i]);
    // }
    // printf("weights:%04x ",weights);
    // volatile float_t *mmio_weights = (volatile float_t *)0xC4000000;
    // //memcpy((void *) mmio_weights, (void *) fbuf, size);
    // for (int i = 0; i < size / sizeof(float_t); i++) {
    //     mmio_weights[i] = ((float_t *)fbuf)[i];
    // }
    // return (float_t *) mmio_weights;
